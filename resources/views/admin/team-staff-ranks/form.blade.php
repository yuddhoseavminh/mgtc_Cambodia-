<div class="flex items-center justify-between border-b border-slate-100 pb-4">
    <div>
        <h3 class="text-xl font-semibold tracking-tight text-slate-900">
            {{ $mode === 'edit' ? 'កែប្រែឋានន្តរស័ក្តិយោធាបុគ្គលិក' : 'បង្កើតឋានន្តរស័ក្តិយោធាបុគ្គលិក' }}
        </h3>
        <p class="mt-1 text-sm text-slate-500">
            {{ $mode === 'edit' ? 'ធ្វើបច្ចុប្បន្នភាពឈ្មោះ លំដាប់ និងស្ថានភាពរបស់ឋានន្តរស័ក្តិយោធាដែលប្រើសម្រាប់បុគ្គលិកក្រុម។' : 'បន្ថែមជម្រើសឋានន្តរស័ក្តិយោធាថ្មីសម្រាប់ទម្រង់ និងតារាងគ្រប់គ្រងបុគ្គលិកក្រុមការងារទី៣ក្រុម។' }}
        </p>
    </div>
    <button type="button" data-staff-rank-modal-close
...
<form
    method="POST"
    action="{{ $mode === 'edit' ? route('team-staff-ranks.update', $rank) : route('team-staff-ranks.store') }}"
    class="mt-6 space-y-5"
    data-action-flow-bound="false"
    data-action-success-title="ជោគជ័យ"
    data-action-success-text="{{ $mode === 'edit' ? 'បានកែប្រែឋានន្តរស័ក្តិយោធាបុគ្គលិកដោយជោគជ័យ។' : 'បានបង្កើតឋានន្តរស័ក្តិយោធាបុគ្គលិកដោយជោគជ័យ។' }}"
>
    @csrf
    @if ($mode === 'edit')
        @method('PUT')
    ...
    {{-- Name --}}
    <div>
        <label class="form-label" for="staff-rank-form-name">
            ឈ្មោះឋានន្តរស័ក្តិ <span class="text-rose-500">*</span>
        </label>
        <input
            type="text"
            name="name_kh"
            id="staff-rank-form-name"
            value="{{ old('name_kh', $rank->name_kh) }}"
            required
            maxlength="255"
            class="form-input bg-[#f8fafc]"
            placeholder="បញ្ចូលឈ្មោះឋានន្តរស័ក្តិយោធា"
            autofocus
        >
    ...
    {{-- Sort Order --}}
    <div>
        <label class="form-label" for="staff-rank-form-sort-order">
            លំដាប់ <span class="text-rose-500">*</span>
        </label>
        <input
            type="number"
            name="sort_order"
            id="staff-rank-form-sort-order"
    ...
    {{-- Status --}}
    <div>
        <label class="form-label" for="staff-rank-form-is-active">
            ស្ថានភាព <span class="text-rose-500">*</span>
        </label>
        <select name="is_active" id="staff-rank-form-is-active" class="form-input bg-[#f8fafc]">
