...
>
    ...
    >
        <div id="level-modal-loader" class="flex flex-col items-center justify-center gap-3 py-12">
        ...
        <div id="level-modal-content" class="hidden"></div>
    </div>
</div>
...
<section class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
    ...
    <button
        type="button"
        data-level-modal-open="{{ route('cultural-levels.create') }}"
                    ...
                    <tr class="border-b border-slate-100 text-sm text-slate-700 last:border-b-0">
                        ...
                        <td class="px-4 py-5 whitespace-nowrap" data-label="សកម្មភាព" data-table-actions>
                            <div class="flex items-center justify-end gap-2">
                                <button
                                    type="button"
                                    data-level-modal-open="{{ route('cultural-levels.edit', $level) }}"
                                ...
                                <form method="POST" action="{{ route('cultural-levels.destroy', $level) }}" data-swal-confirm data-swal-title="បញ្ជាក់ការលុប" data-swal-text="តើអ្នកពិតជាចង់លុបកម្រិតវប្បធម៌នេះមែនទេ?" style="display:contents">
                                    @csrf
                                    @method('DELETE')
...
<script>
(function () {
    var backdrop = document.getElementById('level-modal-backdrop');
    var panel    = document.getElementById('level-modal-panel');
    var loader   = document.getElementById('level-modal-loader');
    var content  = document.getElementById('level-modal-content');
    ...
    function bindModalContent() {
        content.querySelectorAll('[data-level-modal-close]').forEach(function (btn) {
            btn.addEventListener('click', closeModal);
        });
    }

    document.querySelectorAll('[data-level-modal-open]').forEach(function (trigger) {
        trigger.addEventListener('click', function () {
            openModal(trigger.dataset.levelModalOpen);
        });
    });
