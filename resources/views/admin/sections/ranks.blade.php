...
>
    ...
    >
        ...
        <div id="rank-modal-loader" class="flex flex-col items-center justify-center gap-3 py-12">
        ...
        <div id="rank-modal-content" class="hidden"></div>
    </div>
</div>
...
<section class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
    ...
    <button
        type="button"
        data-rank-modal-open="{{ route('ranks.create') }}"
                    ...
                    <tr class="border-b border-slate-100 text-sm text-slate-700 last:border-b-0">
                        ...
                        <td class="px-4 py-5 whitespace-nowrap" data-label="សកម្មភាព" data-table-actions>
                            <div class="flex items-center justify-end gap-2">
                                <button
                                    type="button"
                                    data-rank-modal-open="{{ route('ranks.edit', $rank) }}"
                                ...
                                <form method="POST" action="{{ route('ranks.destroy', $rank) }}" data-swal-confirm data-swal-title="បញ្ជាក់ការលុប" data-swal-text="តើអ្នកពិតជាចង់លុបឋានន្តរស័ក្តិនេះមែនទេ?" style="display:contents">
                                    @csrf
                                    @method('DELETE')
...
<script>
(function () {
    var backdrop = document.getElementById('rank-modal-backdrop');
    var panel    = document.getElementById('rank-modal-panel');
    var loader   = document.getElementById('rank-modal-loader');
    var content  = document.getElementById('rank-modal-content');
            ...
            .catch(function () {
                ...
                if (window.Swal) {
                    Swal.fire({
                        icon: 'error',
                        title: 'មានបញ្ហា',
                        text: 'មិនអាចផ្ទុកទម្រង់បានទេ។ សូមព្យាយាមម្ដងទៀត។',
    ...
    function bindModalContent() {
        content.querySelectorAll('[data-rank-modal-close]').forEach(function (btn) {
            btn.addEventListener('click', closeModal);
        });
    }

    document.querySelectorAll('[data-rank-modal-open]').forEach(function (trigger) {
        trigger.addEventListener('click', function () {
            openModal(trigger.dataset.rankModalOpen);
        });
    });
