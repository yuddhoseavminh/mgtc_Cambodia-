...
>
    ...
    >
        <div id="staff-doc-modal-loader" class="flex flex-col items-center justify-center gap-3 py-12">
        ...
        <div id="staff-doc-modal-content" class="hidden"></div>
    </div>
</div>
...
<section class="dashboard-surface overflow-hidden p-6 sm:p-7">
    <div class="flex flex-col gap-5 border-b border-slate-200 pb-6 lg:flex-row lg:items-end lg:justify-between">
        <div>
            ...
            <h3 class="mt-2 text-[2rem] font-semibold tracking-tight text-slate-950">បញ្ជីឯកសារបុគ្គលិកក្រុមការងារទី៣</h3>
        ...
        <div class="flex items-center gap-3">
            ...
            <button
                type="button"
                data-staff-doc-modal-open="{{ route('team-staff-document-requirements.create') }}"
        ...
        <div class="overflow-x-auto">
            <table class="admin-data-table min-w-full text-left">
                ...
                <tbody>
                        ...
                        <tr class="border-t border-slate-100 text-sm text-slate-700 transition hover:bg-slate-50/70">
                            ...
                            <td class="px-6 py-5 whitespace-nowrap" data-label="សកម្មភាព" data-table-actions>
                                <div class="flex items-center justify-end gap-2">
                                    <button
                                        type="button"
                                        data-staff-doc-modal-open="{{ route('team-staff-document-requirements.edit', $documentRequirement) }}"
                                    ...
                                    <form method="POST" action="{{ route('team-staff-document-requirements.destroy', $documentRequirement) }}" data-swal-confirm data-swal-title="បញ្ជាក់ការលុប" data-swal-text="តើអ្នកពិតជាចង់លុបប្រភេទឯកសារនេះមែនទេ?" style="display:contents">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-16 text-center text-sm text-slate-500">មិនមានប្រភេទឯកសារបុគ្គលិកក្រុមការងារទី៣ទេ។</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
...
<script>
(function () {
    var backdrop = document.getElementById('staff-doc-modal-backdrop');
    var panel    = document.getElementById('staff-doc-modal-panel');
    var loader   = document.getElementById('staff-doc-modal-loader');
    var content  = document.getElementById('staff-doc-modal-content');
    ...
    function bindModalContent() {
        content.querySelectorAll('[data-staff-doc-modal-close]').forEach(function (btn) {
            btn.addEventListener('click', closeModal);
        });
    }

    document.querySelectorAll('[data-staff-doc-modal-open]').forEach(function (trigger) {
        trigger.addEventListener('click', function () {
            openModal(trigger.dataset.staffDocModalOpen);
        });
    });
